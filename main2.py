...
root_path=os.getcwd()+os.sep ## os.sep添加系统分隔符
...
tf.flags.DEFINE_integer("lstm_dim",    100,        "Num of hidden units inLSTM , or num of filters in IDCNN")
...
tf.flags.DEFINE_boolean("pre_emb",     True,       "Wither use pre-trained embedding")#是否使用预训练词向量嵌入
tf.flags.DEFINE_boolean("zeros",       True,       "Wither replace digits with zero")#将数字替换为零
...
tf.flags.DEFINE_string("ckpt_path",    "ckpt",         "Path to save model")
tf.flags.DEFINE_string("summary_path", "summary",      "Path to store summaries")
tf.flags.DEFINE_string("log_file",     "train.log",    "File for log")
tf.flags.DEFINE_string("map_file",     "maps.pkl",     "file for maps")
tf.flags.DEFINE_string("vocab_file",   "vocab.json",   "File for vocab")
tf.flags.DEFINE_string("config_file",  "config_file",  "File for config")
tf.flags.DEFINE_string("script",       "conlleval",    "evaluation script")
tf.flags.DEFINE_string("result_path",  "result",       "Path for results")
tf.flags.DEFINE_string("emb_file",     os.path.join(root_path+"data", "vec.txt"),        "Path for pre_trained embedding")
tf.flags.DEFINE_string("train_file",   os.path.join(root_path+"data", "example.train"),  "Path for train data")
tf.flags.DEFINE_string("dev_file",     os.path.join(root_path+"data", "example.dev"),    "Path for dev data")
tf.flags.DEFINE_string("test_file",    os.path.join(root_path+"data", "example.test"),   "Path for test data")
