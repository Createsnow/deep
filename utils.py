...
import json
import shutil
#在想看的地方写个print xx 就能在控制台上显示打印信息，这样子就能知道它是什么了，但是当我需要看大量的地方或者在一个文件中查看的时候，
# 这时候print就不大方便了，所以Python引入了logging模块来记录我想要的信息。
import logging
...
def create_model(session, Model_class, path, load_vec, config, id_to_char, logger):
    ...
    if ckpt and tf.train.checkpoint_exists(ckpt.model_checkpoint_path):
        ...
        #属于训练，获取新参数（是否加载词向量参数）
        if config["pre_emb"]:
            emb_weights = session.run(model.char_lookup.read_value())
            # emb_weights = model.char_lookup.read_value().eval()
